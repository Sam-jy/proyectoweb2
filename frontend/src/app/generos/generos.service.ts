...
import { IServicioCRUD } from '../compartidos/componentes/interfaces/IServicioCRUD';
...
export class GenerosService implements IServicioCRUD<GeneroDTO, GeneroCreacionDTO>{
  ...
  public obtenerPaginado(paginacion: PaginacionDTO): Observable<HttpResponse<GeneroDTO[]>> {
    let queryParmams = construirQueryParams(paginacion);
    return this.http.get<GeneroDTO[]>(this.urlBase, {params: queryParmams, observe: 'response'});  
  }
